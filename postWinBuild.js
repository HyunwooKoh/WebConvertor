...
const version = versionTokens.reduce((p, c) => p + (p.length ? ',' : '') + parseInt(c[0] === 'v' ? c.slice(1) :  c, 10), '');
...
const resHack = (resolve, reject, args) => {
  ...
  process.on('close', (code) => {
    if (code !== 0) {
      console.log('process ("tool/ResourceHacker.exe '+args.join(' ')+'")\n  are closed: ' + code)
      return reject(-2);
    }
    resolve(code);
  });
...
makeRcFile.then(() => {
  ...
  const inject = new Promise((resolve, reject) =>
    compile.then(() => resHack(resolve, reject, ['-script', 'tool/resource_inject.ini'])).catch(e => reject(e)));
